...
export async function sendEmail(to: string, template: keyof typeof emailTemplates, data: any) {
  try {
    ...
    let templateResult;
    if (template === 'welcome') {
      templateResult = (emailTemplate as any)(data.userName || data.name, data.plan);
    } else if (template === 'paymentSuccess' || template === 'paymentFailed') {
      templateResult = (emailTemplate as any)(data.userName || data.name, data.amount, data.plan);
    } else if (template === 'subscriptionCancelled') {
      templateResult = (emailTemplate as any)(data.userName || data.name, data.plan);
    } else if (template === 'usageLimitWarning') {
      templateResult = (emailTemplate as any)(data.userName || data.name, data.feature, data.current, data.limit);
    } else if (template === 'passwordReset') {
      templateResult = (emailTemplate as any)(data.userName || data.name, data.resetToken);
    } else {
      // Fallback for other templates
      templateResult = (emailTemplate as any)(data.userName || data.name);
    }
...
export async function sendVerificationEmail(userData: { email: string; name: string; verificationToken: string }) {
  ...
  return sendEmail(userData.email, template as any, userData);
} 